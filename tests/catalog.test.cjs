 ...
 *
 * 这个文件是纯数据，但它承担了一个很容易出错的责任：界面上的“总进度 X / 46”“已开放 19 / 46”
 * 全部由它决定。所以这里的断言重点不是“字段存在”，而是三件会真实伤害用户的事：
 *   1. 46 条目录与官方顺序、分组、Project 标记一致，且中英文标题都齐全；
 *   2. available 只在 01-19 为 true —— 一旦有人把 20-46 标成 true，界面就会生成
 *      指向不存在课程页的链接（交接 §6.2 明确禁止）；
 *   3. lessons.js 里绝不会出现 20-46 的正文（交接 §6.1 明确禁止），
 *      同时 01-19 的 slug / 英文标题 / 简体中文标题在两份文件里必须逐字一致，
...
catalog.lessons.forEach(lesson => {
  ...
  assert.equal(
    ...
    `${lesson.slug}: order=${lesson.order} 的 available 应为 ${shouldBeAvailable}（本站只开放 Recipes 之前的 ${AVAILABLE_TOTAL} 课）`
  );
});
...
assert.deepEqual(available.map(l => l.slug), guide.lessons.map(l => l.id), '已开放 19 课的 slug 与 lessons.js 的 id 顺序完全一致');
assert.deepEqual(available.map(l => l.title), guide.lessons.map(l => l.title), '已开放 19 课的英文标题与 lessons.js 逐字一致');
assert.deepEqual(available.map(l => l.zh), guide.lessons.map(l => l.zh), '已开放 19 课的简体中文标题与 lessons.js 逐字一致（不得有第二种译名）');
...
/* §6.1：20-46 课的正文绝不允许出现在 lessons.js 里 */
const guideIds = new Set(guide.lessons.map(l => l.id));
const leaked = catalog.lessons.filter(l => !l.available && guideIds.has(l.slug));
assert.deepEqual(leaked.map(l => l.slug), [], '未开放的 27 课不得在 lessons.js 中出现（§6.1 禁止把 20-46 课正文塞进 lessons.js）');
assert.equal(guide.lessons.length, AVAILABLE_TOTAL, 'lessons.js 仍然只有 19 课中文正文，本轮没有扩展');
...
assert.equal(guide.lessons.length, 19, '已开放分母必须是 19');
assert.ok(guide.lessons.length < catalog.lessons.length, '已开放课程数必须小于总数，否则“19 / 46”就没有意义');
assert.equal(
...
console.log(`通过：官方 Foundations ${CATALOG_TOTAL} 课目录（8 个分组、${projectCount} 个 Project）、order 连续唯一、中英文标题齐全且与 lessons.js 逐字一致、已开放 ${AVAILABLE_TOTAL} 课 / 未开放 ${CATALOG_TOTAL - AVAILABLE_TOTAL} 课、未开放课程未混入 lessons.js、总进度分母 46 与已开放分母 19 不打架、简体中文保险、纯数据无联网、两个 HTML 的加载顺序。`);
