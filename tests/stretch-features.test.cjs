...
/* ===================== 1. I6/I7/I8 目录搜索与过滤 ===================== */
{
  ...
  /* 默认视图 = 完整目录（红线不动） */
  assert.equal(collectByClass(dialog, 'lesson-link').length, 46, check('默认 46 个可点链接'));
  assert.equal(collectByClass(dialog, 'lesson-locked').length, 0, check('默认 0 个未开放锁定行（Foundations 全开）'));
  ...
  setSearch(dialog, '');
  assert.equal(collectByClass(dialog, 'lesson-link').length, 46, check('清空搜索恢复完整目录'));
  ...
  clickChip(dialog, '全部');
  assert.equal(collectByClass(dialog, 'lesson-link').length, 46, check('回到全部恢复 46 链接'));
}
...
/* ===================== 2. I9 命令面板 ===================== */
{
  ...
  assert.ok(filtered.length >= 1 && filtered.length < 44, check('输入 git 过滤生效'));
